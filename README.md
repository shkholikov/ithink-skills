...
![Example output](examples-output/small-office.png)

The legend page is built automatically from the links you actually used:

![Legend page](examples-output/small-office-legend.png)
...
```bash
git clone https://github.com/shkholikov/topology-creator
cp -R topology-creator/skill/topology-creator ~/.claude/skills/
```
...
[Releases](https://github.com/shkholikov/topology-creator/releases) and upload it
at **Settings → Capabilities → Skills**. No dependencies, so it runs unchanged in
Claude's sandbox.
...
```bash
python3 skill/topology-creator/scripts/build_topology.py spec.json -o topology.drawio
```

See [`reference/spec-schema.md`](skill/topology-creator/reference/spec-schema.md)
for the format and
[`reference/examples/small-office.json`](skill/topology-creator/reference/examples/small-office.json)
for a complete worked spec.
...
All 294 icons are also addressable by their own slug — see
`skill/topology-creator/assets/icons.json`. An unmatched name renders as a dashed
placeholder box and prints a warning; it never fails the build.
...
```bash
pip install pillow
python3 build/convert_icons.py "cisco icons" skill/topology-creator/assets/icons
```
...
The network icons in `skill/topology-creator/assets/icons/` are derived from the
**Cisco Network Topology Icon** library and remain the property of Cisco Systems,
Inc. They are included here for convenience and are not covered by the MIT
licence. Cisco provides them for use in documentation and presentations; refer to
Cisco's own terms for what you may do with them.
